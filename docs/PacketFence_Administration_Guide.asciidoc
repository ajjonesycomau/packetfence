...
CAUTION: You will need to disable password hashing in the database for local authentication to work. In the administration interface, go in 'Configuration -> Advanced' and set 'Database passwords hashing method' to `plaintext` or `ntlm`. Don't use `bcrypt`.
...
In order to use the passthroughs feature in PacketFence, you need to enable it from the GUI in *Configuration->Trapping*, enable *Passthrough* and then save.
...
Once done, add `net.ipv4.ip_forward=1` in `/etc/sysctl.conf` and reload it using `sysctl -p`. This enables packet routing in the kernel.

Then add passthroughs in *Configuration->Trapping->Passthroughs*. They can be of the following format:
...
In addition to the options above, you can prefix the domain with `*.` (*.example.com) to whitelist all the subdomains of example.com (ex: `www.example.com`, `my.example.com`).
...
The proxy passthroughs can be configured in *Configuration->Trapping->Proxy Passthroughs*.
...
Once this is done, you need to restart the radiusd-dhcpd and pfdns services.
...
* `group_field` : The field to group the entries by. No grouping is done if this field is omited or empty.
...
NOTE: You should always prefix the fields with the table name and a dot (ex: `node.mac`, `locationlog.role`, ...) so that they are not ambigious. Although your query may work with a single table, it will not if you decide to add joins that contain column name(s) that are the same as the base table.
...
NOTE: This section assumes that you already have an Active Directory domain configuration both in 'Configuration->Domains' and 'Configuration->Sources'. If you don't, you need to first configure those. Refer to the appropriate sections of this guide for details on how to configure those two components.
...
In order to use this cache, you will need to install additionnal packages available from the packetfence-extra repository. These packages will allow PacketFence to query your Active Directory for the NT hash of the users.
...
First of all, you will need to enable the NTLM caching globally by enabling 'NTLM Redis cache' in 'Configuration->Advanced'. You then need to restart radiusd.

Once that is done, you need to configure PacketFence to start caching the credentials. In order to do so, go in 'Configuration->Domains' and select the domain you want to cache the credentials for. 
...
After that, you will need to enable the `redis_ntlm_cache` service which is used by PacketFence to store the cached credentials. In order to do so, go in 'Configuration->Services' and enable 'redis_ntlm_cache' and save the changes.

Next, start the service via pfcmd:

  /usr/local/pf/bin/pfcmd service redis_ntlm_cache start

If you chose to enable 'NTLM cache background job' in one of your domains, you will need to enable the pfmon job that will periodically cache the credentials. This can be configured in 'Configuration->Maintenance->NTLM cache job interval'. It is advised to set this to half the expiration of the credentials you have set in the domain configuration. This will ensure you have an optimal cache hit. Once done, restart the `pfmon` service.
...
The installation instructions below are made for CentOS 6 but adjusting them to CentOS 7 or Debian should only be a matter of installing the proper packages for your MySQL/MariaDB version.
...
Next, install Percona XtraBackup by manually specifying the Percona repository:
  ...
  # yum install percona-xtrabackup-20 --enablerepo=percona-release-`uname -m`

NOTE: A more recent version of Percona XtraBackup can be used but only if you are running a recent version of MySQL.
